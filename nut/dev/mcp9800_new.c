...
int mcp9800GetTemperature(void *temperature)
{
    ...
    return TwMasterRegRead(MCP9800_SLAVE_ADDRESS, MCP9800_TEMPERATURE, 1, temperature, 2, MCP9800_TIMEOUT);
}
...
int mcp9800IOCtl(int cmd, void *conf)
{
	int rc = 0;
	uint8_t data;
	if(TwMasterRegRead(MCP9800_SLAVE_ADDRESS, MCP9800_GET_CONF, 1, &data, 1, MCP9800_TIMEOUT) == -1){
	    return -1;
	}
	...
	case MCP9800_SET_CONF_RESOLUTION:
		...
		TwMasterRegWrite(MCP9800_SLAVE_ADDRESS, MCP9800_GET_CONF, 1, &data, 1, MCP9800_TIMEOUT);
		break;
	default:
		rc = -1;
		break;
