...
static void EeBusy(void)
{
	...
	while(TwMasterRegWrite(I2C_SLA_24AA256, 0, 0, 0, 0, EEPROM_BUSY_TIMEOUT) != 0 && i-- != 0);
}
/*!
...
int EEReadBlock(uint16_t addr, void *rxdata, uint16_t rxsiz, uint32_t tmo)
{
	EeBusy();
	if(rxsiz > 0){
		return TwMasterRegRead(I2C_SLA_24AA256, addr, 2, rxdata, rxsiz, tmo);
	}
	return 0;
}
...
int EEWriteBlock(uint16_t addr, CONST void *txdata, uint16_t txsiz)
{
	...
	while(txsiz > 0){
		EeBusy();
		if (TwMasterRegWrite(I2C_SLA_24AA256, addr, 2, (void *)p_txdata, len, EEPROM_RW_TIMEOUT) < 0)
			return -1;
		txsiz -= len;
		p_txdata += len;
		addr += len;
...
{
	EeBusy();
	return TwMasterRegRead(I2C_SLA_24AA256, addr, 2, rxdata, 1, EEPROM_RW_TIMEOUT);
}
...
{
	EeBusy();
	return TwMasterRegWrite(I2C_SLA_24AA256, addr, 2, (void *)txdata, 1, EEPROM_RW_TIMEOUT);
}
